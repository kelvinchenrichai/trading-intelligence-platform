...
export const CME_PARSER_VERSION = "cme-pg40-v0.3.1-safe-optiontype-geometry";
...
function optionTypeFromText(text: string, fallback: CmeOptionType = "call"): CmeOptionType {
  if (/\bPUTS?\b/.test(text) || /\bP\b/.test(text)) return "put";
  if (/\bCALLS?\b/.test(text) || /\bC\b/.test(text)) return "call";
  return fallback;
}
...
function detectContext(line: PdfLine, current: Context | null): Context | null {
  ...
  if (isSectionHeader && /NASDAQ|NASD|NSDQ/.test(text) && !/RUSSELL|RTY|MICRO/.test(text) && /(CALLS|PUTS|\bCALL\b|\bPUT\b|EOM)/.test(text)) {
    const optionType: CmeOptionType = optionTypeFromText(text, current?.optionType ?? "call");
...
export async function parseCmeSection40(pdfBuffer: Buffer, fileName: string): Promise<CmeNqImportResult> {
  ...
  const warnings = [
    ...
    "CME PG40 v5 parser stores CME OI, settlement, volume, CME-published delta, and full-expiry audit rows. v0.3.1 uses safe geometry parsing with corrected PUT/CALL context; Black-76 GEX and comparable display calibration are computed after import.",
  ];
  return {
